...
We now need to create a table with columns: century, cumulative extinction  percent, class
```{r}
...
cumulative<-combined|>
  ...
  left_join(total_sp)|>
  group_by(class)|>
  mutate(percent_extinct=(n/total)*10000) |>
  mutate(cumulative_extinct=(cumsum(n))) |>
  mutate(percent_cumulative_extinct=(cumulative_extinct/total)*10000) 
cumulative
